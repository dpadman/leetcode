...
private:
    ...
    void traverse(TreeNode *root, vector<vector<vector<int>>>& ans) {
        queue<qE> Q;
        if (!root)
            return;
        qE qe;
        ...
        while (!Q.empty()) {
            qe = Q.front();
            Q.pop();
            ans[qe.X][qe.Y].push_back(qe.r->val);
            sort(ans[qe.X][qe.Y].begin(), ans[qe.X][qe.Y].end());
            if (qe.r->left) {
                qE qel;
                qel.r = qe.r->left;
                qel.X = qe.X-1;
...
public:
    vector<vector<int>> verticalTraversal(TreeNode* root) {
        ...
        vector<vector<vector<int>>> ans(2001, vector<vector<int>>(1001, vector<int>()));
        traverse(root, ans);
        for (int i=0; i<2000; i++) {
            ...
            for (int j=1000; j>=0; j--) {
                for (int k=0; k<ans[i][j].size(); k++) {
                    ...
                    ret[ret.size()-1].push_back(ans[i][j][k]);
                }
            }
        }
        return ret;
